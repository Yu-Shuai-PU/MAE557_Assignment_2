      ...
      \begin{subequations}
        ...
        \begin{subequations}
          ...
          \begin{subequations}
            ...
            In this work, we will choose the forward Euler method as the timestepper, since it is easy to implement in our code with a first-order temporal accuracy.
            ...
            \end{equation}
            As we know, for a 1D linear advection PDE $\partial_t u + a\partial_x u = 0$ with the second-order central scheme for spatial derivatives and the forward Euler timestepper, the CFL number $a\Delta t/\Delta x$ has a limit of $1$.
            So, we can utilize this conclusion to give the same form of upper bound for our CFL number:
            \begin{align}
              \label{eq:48}
              C \leq 1 \Longrightarrow & \frac{(|u_1|_{\max} + a_{\max})\Delta t}{\Delta x_1} + \frac{(|u_2|_{\max} + a_{\max})\Delta t}{\Delta x_2}\leq 1,\notag\\
              \Longrightarrow & \frac{(|u_1^*|_{\max} + \mathrm{Ma}^{-1}\sqrt{T^*_{\max}})\Delta t^*}{\Delta x_1^*} + \frac{(|u_2^*|_{\max} + \mathrm{Ma}^{-1}\sqrt{T^*_{\max}})\Delta t^*}{\Delta x_2^*}\leq 1.
            \end{align}
            ...
            \begin{algorithm}
              \caption{FTCS Solver for 2D Compressible Lid-Driven Cavity Flow}
...
\begin{algorithmic}[1]
  ...
  \For{$j = 0$ to $N_t$} (\textbf{the forward Euler scheme})
    ...
    \State Evaluate the CFL number $C \gets \frac{(|u_1^*|_{\max} + \mathrm{Ma}^{-1}\sqrt{T^*_{\max}})\Delta t^*}{\Delta x_1^*} + \frac{(|u_2^*|_{\max} + \mathrm{Ma}^{-1}\sqrt{T^*_{\max}})\Delta t^*}{\Delta x_2^*}$.
        \If{$C > 1$}
            ...
            \State \textbf{Exit}.
        \EndIf
    \State $\mathbf{U}(t^* + \Delta t^*) \gets \mathbf{U}(t^*) + \Delta t^* \cdot \text{RHS}(\mathbf{U}(t^*), t^*)$
    \EndFor
...
We study the temporal convergence via fixing $N_x = N_y = 33$ (including both boundary points) and setting $\Delta t^* = 2\times 10^{-5}, 10^{-5}, 5\times 10^{-6}$.
The error is evaluated as the root-mean-square of difference between the numerical solutions and a high-fidelity solution $\tilde{u}$ solved with $\Delta t^* = 10^{-6}$ on each grid point at the final time $t_{f}^* = 0.2$:
\begin{equation}
  \label{eq:28}
  \epsilon = \frac{1}{N_{x_1}N_{x_2}}\sum_{i = 0}^{N_{x_1}-1}\sum_{j = 0}^{N_{x_2}-1}|\bm{u}^*(i\Delta x_1^*,j\Delta x_2^*, t_f^*) - \tilde{\bm{u}}^*(i\Delta x_1^*,j\Delta x_2^*, t_f^*)|_2^2,
\end{equation}
where $\bm{u}^* = [\rho^*, u_1^*, u_2^*, T^*]$.
Fig.~\ref{fig:convergence_rate}(a) shows the rate of convergence for our forward-Euler temporal discretization, which is consistent with the temporal accuracy of the scheme.

Similarly, we study the spatial convergence via fixing $\Delta t^* = 10^{-5}$ and choose $N_x = N_y = 9, 17$ and $33$, respectively.
The error is then evaluated as the pointwise difference between the numerical solutions and a high-fidelity solution $\tilde{u}$ solved with $N_x = N_y = 65$ on the fixed location $(x_1^*, x_2^*) = (15/16, 15/16)$ at $t_{f}^* = 1.0$:
\begin{equation}
  \label{eq:28}
  \epsilon = |\bm{u}^*(15/16, 15/16, t_f^*) - \tilde{\bm{u}}^*(15/16, 15/16, t_f^*)|_2^2.
\end{equation}
Fig.~\ref{fig:convergence_rate}(b) shows the rate of convergence for our central spatiall discretization, which is consistent with the second-order accuracy of the scheme.

\begin{figure}[tbp]
  \centering
  \begin{tikzonimage}[width=0.4\linewidth]{temporal_convergence_rate.png}%[tsx/show help lines]
    \node at (0.55, -0.05) {(a)};
  \end{tikzonimage}
  \begin{tikzonimage}[width=0.4\linewidth]{spatial_convergence_rate.png}%[tsx/show help lines]
    \node at (0.55, -0.05) {(b)};
  \end{tikzonimage}
  \caption{Temporal and spatial rate of convergence for the FTCS solver of the cavity flow problem.}
  \label{fig:convergence_rate}
\end{figure}

\section{Qualitative changes of solutions and computational efficiency with increasing Mach number}
